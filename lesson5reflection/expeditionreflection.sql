...
-- При решении данной задачи было допущенно несколько ошибок:
-- 1) Стоило использовать with, что в даннос случае позволило бы разделить запрос на несколько подзапросов,
-- внутри которых удобно использовать join и группировки.
-- 2) Стоило использовать COALESCE, чтобы вывести 0 в случае, если в таблице, используемой для рассчетов,
-- не окажется строк для выбранной экспедиции.
-- 3) Стоило использовать конструкцию sum(case when) для подсчета вместо подзапроса с условием.